...
/* ============================== shared UI ============================== */
const CARD = "rounded-2xl border border-black/[0.06] dark:border-white/[0.08] bg-white/70 dark:bg-white/[0.035] " +
    "backdrop-blur-xl shadow-[0_1px_2px_rgba(16,16,20,.04),0_10px_34px_-12px_rgba(16,16,20,.14)] dark:shadow-none";
const GRAD = "bg-gradient-to-r from-indigo-500 via-violet-500 to-fuchsia-500";
const TipCtx = createContext(() => { });
function Tooltip({ tip }) {
    return (_jsx(AnimatePresence, { children: tip && (_jsxs(motion.div, { initial: { opacity: 0, scale: 0.96 }, animate: { opacity: 1, scale: 1 }, exit: { opacity: 0, scale: 0.96 }, transition: { duration: 0.12 }, style: { left: tip.x, top: tip.y }, className: "pointer-events-none fixed z-50 max-w-[320px] rounded-xl border border-black/10 dark:border-white/10\n                     bg-white/95 dark:bg-[#1c1c20]/95 backdrop-blur-xl px-3 py-2 text-[12.5px] leading-snug\n                     text-ink shadow-xl", children: [tip.title && _jsx("div", { className: "mb-0.5 text-[11px] text-muted", children: tip.title }), _jsx("div", { dangerouslySetInnerHTML: { __html: tip.body } })] })) }));
}
function useTip() {
...
function AnimatedNumber({ value, decimals = 0, sign = false, duration = 1.1 }) {
    ...
    return _jsxs(_Fragment, { children: [sign && value >= 0 ? "+" : "", s] });
}
...
function SentimentLine({ points, width = 980, height = 250 }) {
    ...
    const onMove = (e) => {
        ...
        setFocus(best);
        setTip({ x: e.clientX + 16, y: e.clientY + 16, title: `${best.full} · ${sentWord(best.y)} ${signed(best.y)}`, body: best.story });
    };
...
function LockScreen({ onUnlocked }) {
    ...
    const submit = async (e) => {
        ...
        try {
            ...
            sessionStorage.setItem("hfl_pass", pass.trim());
            onUnlocked(data);
        }
        catch {
...
function MonthPicker({ value, options, labels, onChange }) {
    ...
    return (_jsxs("div", { className: "relative", ref: ref, children: [_jsxs(motion.button, { onClick: () => setOpen((o) => !o), whileTap: { scale: 0.98 }, className: `${CARD} flex min-w-[190px] items-center justify-between gap-3 px-4 py-2.5 text-[14px] font-medium`, children: [labels[value], _jsx(motion.span, { animate: { rotate: open ? 180 : 0 }, transition: { duration: 0.2 }, className: "text-muted", children: "\u25BE" })] }), _jsx(AnimatePresence, { children: open && (_jsx(motion.ul, { initial: { opacity: 0, y: -6, scale: 0.97 }, animate: { opacity: 1, y: 0, scale: 1 }, exit: { opacity: 0, y: -6, scale: 0.97 }, transition: { duration: 0.16, ease: EASE }, className: `${CARD} absolute z-30 mt-2 w-full overflow-hidden p-1.5`, children: options.map((o) => (_jsx("li", { children: _jsxs("button", { onClick: () => { onChange(o); setOpen(false); }, className: `flex w-full items-center justify-between rounded-lg px-3 py-2 text-left text-[14px] transition
                              hover:bg-black/[0.05] dark:hover:bg-white/[0.07] ${o === value ? "font-semibold" : "text-ink2"}`, children: [labels[o], o === value && _jsx("span", { className: "text-violet-500", children: "\u2713" })] }) }, o))) })) })] }));
}
...
function AnalyticsTab({ data, themeOf, months, monthKeys }) {
    ...
    const points = useMemo(() => entries.map((e) => ({ x: new Date(e.date + "T00:00:00"), y: e.sentiment, story: e.story, full: dayParts(e.date).full })), [entries]);
    const words = useMemo(() => {
...
function App() {
    ...
    useEffect(() => {
        ...
        if (saved)
            unlock(saved).then(setData).catch(() => sessionStorage.removeItem("hfl_pass"));
    }, []);
